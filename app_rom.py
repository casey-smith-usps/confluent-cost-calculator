...
DEFAULT_ROM_CONFIG = {
    ...
    'num_topics': 1,   # Total number of topics across all ingests (cost driver)
...
if st.session_state.show_rom_settings:
    ...
    with vol_col2:
        num_ingests_val = st.number_input(
            "Number of Ingests",
            ...
            help="How many separate ingest connectors/pipelines"
        )
        st.session_state.rom_config['num_ingests'] = num_ingests_val

    with vol_col3:
        # Default num_topics to num_ingests if not set or if ingests just changed
        default_topics = st.session_state.rom_config.get('num_topics', num_ingests_val)
        num_topics_val = st.number_input(
            ...
            help="Sum of all topics across all ingests (e.g., 2 ingests with 3 and 4 topics = 7). Topics are the cloud cost driver."
        )
        ...
        if num_topics_val == num_ingests_val:
            ...
            avg = num_topics_val / num_ingests_val if num_ingests_val > 0 else num_topics_val
            st.caption(f"{avg:.1f} topics per ingest avg")
    ...
    # Initialize feed_configs if not present — sized by num_topics (cost driver)
    if 'feed_configs' not in st.session_state.rom_config or len(st.session_state.rom_config['feed_configs']) != st.session_state.rom_config['num_topics']:
        st.session_state.rom_config['feed_configs'] = [
            {'inbound': 1, 'outbound': 1, 'partitions': 0.048}
            for _ in range(st.session_state.rom_config['num_topics'])
        ]
...
def calculate_costs(size_config, selected_size, num_topics=1, records_per_day=5000):
    # Calculate total CKU cost
    azure_annual = st.session_state.cku_config['azure_ckus'] * st.session_state.cku_config['azure_rate'] * 12
    gcp_annual = st.session_state.cku_config['gcp_ckus'] * st.session_state.cku_config['gcp_rate'] * 12
    total_cku_cost_annual = azure_annual + gcp_annual

    # Prorate costs based on resource utilization per topic (topics are the cost driver)
...
num_topics = st.session_state.rom_config.get('num_topics', num_ingests)
...
if st.session_state.show_technical_model:
    ...
    **Current ROM Configuration:**
    ...
    - Total Partitions: {size_config['partitions'] * num_topics:.0f}
    - Total Records per Day: {records_per_day * num_topics:,}
    - Per-Topic Records per Day: {records_per_day:,}
    """)

    if st.button("🔄 Sync from ROM", use_container_width=True, type="primary"):
        # Calculate technical defaults from ROM - scale by number of topics (cost driver)
        total_partitions = size_config['partitions'] * num_topics

        # Scale records by number of topics
        total_records_per_day = records_per_day * num_topics
        ...
        st.success(f"✅ Technical Model synced from ROM configuration ({num_ingests} ingests, {num_topics} topics)!")
        st.rerun()
...
with col1:
    ...
    st.info(f"""
    **📈 Utilization & Scaling:**
    - Compute: {(size_config['partitions'] / TOTAL_PARTITIONS) * 100:.3f}% per topic × {num_topics} topics
    - Storage: {(size_config['storage_gb'] / TOTAL_STORAGE_GB) * 100:.3f}% per topic × {num_topics} topics
    - Network: {costs['partition_utilization']:.2f}% utilization ({size_config['partitions'] * num_topics:.2f} total partitions)
    - Ingests: {num_ingests} | Topics: {num_topics} | Records/day: {records_per_day:,}
    """)
...
with rom_col3:
    st.markdown(f"""
        <div class="metric-card" style="border-left-color: #FF9800;">
            ...
            <hr style="border-color: #ddd; margin: 0.5rem 0;">
            <p style="font-size: 0.85rem; margin: 0;">
                {rom_results['total_feeds']} ingests | {rom_results['num_topics']} topics | {rom_results['partition_utilization_pct']:.2f}% network
            </p>
        </div>
    """, unsafe_allow_html=True)

st.info(f"""
**ROM Configuration:** {rom_results['total_feeds']} ingest(s) | {rom_results['num_topics']} total topic(s) | {rom_results['records_per_day']:,} records/day | {size_config['partitions']:.2f} partitions per topic (T-shirt size: {selected_size})

**Pricing Method:** CKU-based (actual infrastructure costs)

**Cost scales with:** Total number of topics ({rom_results['num_topics']}), T-shirt size ({selected_size}), partition usage ({rom_results['partition_utilization_pct']:.2f}%), and daily volume ({rom_results['records_per_day']:,} records)
""")
...
with col2:
    if st.button("📥 Export Reports", use_container_width=True, type="primary"):
        ...
        # Sized by num_topics — topics are the cost driver
        export_rom_config['feed_configs'] = [
            ...
            for _ in range(st.session_state.rom_config.get('num_topics', st.session_state.rom_config.get('num_ingests', 1)))
        ]
...
with st.expander("📐 Formula Reference"):
    ...
    **Notes:**
    - Number of Ingests drives DE engineering hours (pipeline build work)
    - Total Number of Topics drives all cloud costs (partitions, storage, network, governance)
